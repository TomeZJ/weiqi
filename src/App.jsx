...
function App() {
      ...
      starPositions.forEach((x) => {
        starPositions.forEach((y) => {
          if (x < boardSize && y < boardSize) {
            ctx.beginPath();
            ctx.arc(
              20 + x * cellSize.current,
              20 + y * cellSize.current,
              4,
              0,
              2 * Math.PI,
            );
  ...
  const canPlaceStone = (x, y) => {
    ...
    for (const [dx, dy] of directions) {
      ...
      if (nx >= 0 && nx < boardSize && ny >= 0 && ny < boardSize) {
        if (newBoard[ny][nx] === oppositeColor) {
          const oppLiberties = calculateLiberties(
            nx,
            ny,
            oppositeColor,
            newBoard,
    ...
    if (capturedCount === 1 && capturePosition) {
      // 检查提子后是否形成打劫
      const { newBoard: boardAfterCapture } = removeCapturedStones(
        newBoard,
        oppositeColor,
      );
      // 模拟对方回提
      ...
      const tempLiberties = calculateLiberties(
        cx,
        cy,
        oppositeColor,
        tempBoard,
  ...
  const getCoordinateLabel = (x, y) => {
    const letter = String.fromCharCode(65 + x); // 65是'A'的ASCII码
  ...
  const handleClick = (e) => {
    ...
    if (x >= 0 && x < boardSize && y >= 0 && y < boardSize) {
      if (canPlaceStone(x, y)) {
        ...
        // 提对方棋子
        const oppositeColor = currentPlayer === "black" ? "white" : "black";
        const { newBoard: boardAfterCapture, captured } = removeCapturedStones(
          newBoard,
          oppositeColor,
        );
        ...
        // 检查己方是否有被提的棋子
        const { newBoard: finalBoard } = removeCapturedStones(
          boardAfterCapture,
          currentPlayer,
        );
      ...
      switch (robotLevel) {
        ...
        case "hard":
          ...
          possibleMoves.forEach(([x, y]) => {
            ...
            // 计算气数
            const liberties = calculateLiberties(
              x,
              y,
              currentPlayer,
              tempBoard,
    ...
    <div className="app">
      ...
      <div className="controls">
        <div className="player-info">
          <div
            className={`player ${currentPlayer === "black" ? "active" : ""}`}
          >
            <span className="stone black"></span>
            <span>黑棋: {blackCount}</span>
          </div>
          <div
            className={`player ${currentPlayer === "white" ? "active" : ""}`}
          >
      ...
      <div className="game-container">
        <div className="board-container">
          <canvas ref={canvasRef} onClick={handleClick} className="board" />
        </div>
